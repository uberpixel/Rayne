    ...
    foreach(_RESOURCE ${_RESOURCES})
        if(ANDROID)
            if(IS_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}")
                add_custom_command(TARGET ${_TARGET} PRE_BUILD COMMAND ${PYTHON_EXECUTABLE} "${DIR_OF_RAYNE_CMAKE}/../Tools/ResourcePacker/pack.py" "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}" "${android-assets-dir}/${_RESOURCE}" mobile ${_ADDITIONAL_PACK_PARAMS})
            else()
        ...
        elseif(APPLE)
            if(IS_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}")
                add_custom_command(TARGET ${_TARGET} PRE_BUILD COMMAND ${PYTHON_EXECUTABLE} "${DIR_OF_RAYNE_CMAKE}/../Tools/ResourcePacker/pack.py" "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}" "$<TARGET_BUNDLE_CONTENT_DIR:${_TARGET}>/Resources/${_RESOURCE}" pc ${_ADDITIONAL_PACK_PARAMS})
            else()
            ...
            if(IS_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}")
                add_custom_command(TARGET ${_TARGET} PRE_BUILD COMMAND ${PYTHON_EXECUTABLE} "${DIR_OF_RAYNE_CMAKE}/../Tools/ResourcePacker/pack.py" "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}" "$<TARGET_FILE_DIR:${_TARGET}>/${_RESOURCE}" pc ${_ADDITIONAL_PACK_PARAMS})
            else()
