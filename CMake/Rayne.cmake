...
macro(rayne_use_modules _TARGET _MODULES)
    foreach(_MODULE ${_MODULES})
        ...
        if(ANDROID)
            ...
            cmake_path(CONVERT ${android-assets-dir} TO_CMAKE_PATH_LIST ANDROID_ASSETS_DIR NORMALIZE) #turn into cmake path with forward-slashes (fixing things on windows)
            INTERNAL_rayne_copy_module_resources(${_TARGET} ${_MODULE} ${ANDROID_ASSETS_DIR}/Resources/Modules)
        endif()
    endforeach()
endmacro()
        ...
        elseif(APPLE)
            if(IS_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}")
                if(IOS OR VISIONOS)
                    ...
                    add_custom_command(TARGET ${_TARGET} PRE_BUILD COMMAND ${Python_EXECUTABLE} "${DIR_OF_RAYNE_CMAKE}/../Tools/ResourcePacker/pack.py" "${CMAKE_CURRENT_SOURCE_DIR}/${_RESOURCE}" "$<TARGET_BUNDLE_CONTENT_DIR:${_TARGET}>/Resources/${_RESOURCE}" macos ${_ADDITIONAL_PACK_PARAMS})
                endif()
            else()
                if(IOS OR VISIONOS)
