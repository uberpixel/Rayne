...
namespace RN
{
	...
	void InstancingNode::WillRemoveChild(SceneNode *child)
	{
		if(child->IsKindOfClass(_entityClass))
		{
			...
			entity->RemoveFlags(Flags::Hidden);
	...
	bool InstancingNode::CanRender(Renderer *renderer, Camera *camera) const
	{
		return true;
	}

	void InstancingNode::Render(Renderer *renderer, Camera *camera) const
	{
		for(InstancingData *data : _rawData)
